...
class Interface
  ...
  def run
    mthds =
      %i[exit create_station create_train create_route
         add_station_to_route delete_station_from_route
         add_route_to_train edit_wagons change_directory
         show_stations show_trains_on_station show_wagons occupite]
    loop do
      yield
      choice = gets.to_i
      send(mthds[choice])
